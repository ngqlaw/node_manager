...
start_handler() ->
    ...
    Handlers = application:get_env(?NODE_APP, server_ext_handle, []),
    Handles = [
        begin
...
handle_event(Event, State) ->
    lager:debug("Server event:~p", [Event]),
    {ok, State}.
...
handle_call(Request, State) ->
    lager:debug("Server call:~p", [Request]),
    Reply = ok,
    {ok, Reply, State}.
...
handle_info(Info, State) ->
    lager:debug("Server info:~p", [Info]),
    {ok, State}.
